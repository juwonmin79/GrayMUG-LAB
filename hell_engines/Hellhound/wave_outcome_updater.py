...
try:
    from .mfe_mae_engine import calculate_mae, calculate_mfe, calculate_time_to_peak, calculate_time_to_stop
except ImportError:
    from mfe_mae_engine import calculate_mae, calculate_mfe, calculate_time_to_peak, calculate_time_to_stop
...
def create_wave_outcome_update(
    ...
    *,
    ...
    if updated.get("signal_id") and not updated.get("shadow_signal_id"):
        updated["shadow_signal_id"] = updated["signal_id"]
    ...
    for window in OUTCOME_WINDOWS:
        ...
        if entry is None or not price_path:
            ...
            updated[f"outcome_time_to_stop_{suffix}"] = None
            continue
        ...
        updated[f"outcome_time_to_stop_{suffix}"] = calculate_time_to_stop(
            entry,
            price_path,
